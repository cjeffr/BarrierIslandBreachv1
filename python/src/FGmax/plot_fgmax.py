...
def plot_fgmax_grid(fname,fgno):

    fg = fgmax_tools.FGmaxGrid()
    fg.read_input_data(fname)
    fg.read_output(fgno=fgno)

    clines_zeta = [0.01] + list(numpy.linspace(0.05,0.3,6)) + [1.0,2.0,3.0]
    ...
    plt.colorbar()
    plt.contour(fg.X,fg.Y,fg.B,[0.],colors='k')  # coastlin
    ...
    plt.gca().set_aspect(1./numpy.cos(fg.Y.mean()*numpy.pi/180.)) #.804482897/.3623188)#
    print(1./numpy.cos(fg.Y.mean()*numpy.pi/180.))
    plt.title("Maximum amplitude")


if __name__=="__main__":
    import os
    plot_fgmax_grid('breach_fgmax.txt',1)
    plotdir = '_plots'
    if not os.path.isdir(plotdir): 
        os.mkdir(plotdir)
    oname = os.path.join(plotdir, "breach_max_a.png")
